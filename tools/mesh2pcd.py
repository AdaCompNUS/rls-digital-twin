...
def convert_mesh_to_pointcloud(
    ...
    if blue_noise and len(points) > 0:
        ...
        filtered, _ = downsampled.remove_statistical_outlier(nb_neighbors=20, std_ratio=2.0)
    ...
    if uniform_density and len(points) > 0:
        ...
        distances, _ = kdtree.query(points, k=k_neighbors+1)  # +1 because first is self
        ...
        if len(equalized_points) > len(points)/3:  # Ensure we don't remove too many points
    ...
    if visualize and len(points) > 0:
        ...
        o3d.visualization.draw_geometries([pcd, coord_frame],
                                         window_name="Uniform Point Cloud Result")
    
    return points


def main():
    parser = argparse.ArgumentParser(description='Convert mesh to uniform point cloud for motion planning')
    parser.add_argument('input', help='Input mesh file path (DAE, OBJ, STL, etc.)')
    parser.add_argument('-o', '--output', help='Output point cloud file path (PLY format)')
    parser.add_argument('-v', '--voxel-size', type=float, default=0.02,
                        help='Voxel size for sampling resolution (default: 0.02)')
    parser.add_argument('--no-poisson', action='store_true',
                        help='Disable Poisson disk sampling')
    parser.add_argument('--no-blue-noise', action='store_true',
                        help='Disable blue noise sampling')
    parser.add_argument('--no-uniform-density', action='store_true',
                        help='Disable density equalization')
    parser.add_argument('--visualize', action='store_true',
                        help='Visualize the resulting point cloud')
